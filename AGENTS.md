...
## 지출 입력 경로는 카드 SMS + 수기 입력만

이 앱이 자동화/매칭 대상으로 다루는 지출 입력 경로는 **카드 결제 SMS 자동입력**과 **사람이 직접 입력하는 수기 작성** 두 가지뿐이다. 계좌이체(계좌간 이동, 급여 입금 등)는 신경 쓰지 않는다 — 고정지출 상호명 매칭, 소분류 자동 카테고리 매칭 등 새 자동화 기능을 설계할 때 계좌이체 감지·자동화는 만들지 않는다(문자 형식이 카드 결제와 다르고, 오탐 위험만 키움). 은행 SMS 자체를 검토 대기함에서 거르지 않고 받는 기존 동작(이체 문자도 일단 `pending_review`로 저장, 사람이 승인/삭제 판단)은 계속 유지 — 이건 "감지 자동화"가 아니라 사람이 최종 판단하는 fallback이라 별개.
